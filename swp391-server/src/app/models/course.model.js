...
const searchCourseByName = async (course_name) => {
    ...
    const [rows] = await db.execute('SELECT c.course_id, c.course_name, c.created_at, c.created_by, c.video, c.quiz, c.version FROM Course c WHERE course_name LIKE ? AND is_active = 1',
        [name]
    )
    return rows;
}
...
const memberContinuesLearnCourseById = async (member_id, course_id) => {
    const [rows] = await db.execute('SELECT c.course_id, u.fullname, c.course_name, ce.learning_process, c.content, c.version  FROM Users u  JOIN  Course_enrollment ce ON u.user_id = ce.member_id JOIN Course c ON ce.course_id = c.course_id WHERE u.user_id = ? AND ce.course_id = ? AND ce.is_active = 1 ',
        [member_id, course_id]
    )
    return {
